...
import AllSales from '../Table/AllSales';
function AllSale({ allSales, count, filter, filterUsers, setFilter, isLoading }) {
    ...
    const { owners, sales, formattedData } = useMemo(() => {
        if (!allSales) return { owners: [], sales: [], formattedData: [] };

        return allSales.reduce(
            (acc, l) => {
                acc.owners.push(l.day);
                acc.sales.push(l.total_sales);
                acc.formattedData.push({ x: l.day, y: l.total_sales });
                return acc;
            },
    ...
    const renderChart = () => {
        ...
        switch (filter?.allSaleFilter?.chart) {
            case "column":
            ...
            default:
                return <AllSales allSales={allSales} isLoading={isLoading} />;
        }
    }
    return (
        <div>
            <div className='flex justify-between items-center  bg-[#dbe7ff38] p-2 rounded'>
                ...
                <div className='flex gap-4'>
                    <select
                        ...
                        value={filter?.allSaleFilter?.manager}
                    ...
                    {filter?.allSaleFilter?.manager != "0" && (
                        <select
                            className='form-select form-select-sm w-[150px]'
                            value={filter?.allSaleFilter.leader}
                            onChange={(e) => handleFilter({ action: 'leader', value: e.target.value })}
                        >
                            <option value="0">All</option>
                            {filterUsers?.filter(user => user.user_type == "Team Leader" && user.manager_id == filter?.allSaleFilter?.manager).map(leader => (
                ...
                <div className='flex gap-5 mt-1'>
                    <label className="inline-flex">
                        <input
                            ...
                            checked={filter?.allSaleFilter?.isActiveUsers === "active"}
                    ...
                    <label className="inline-flex">
                        <input
                            ...
                            value={'inactive'}
                            className="form-radio"
                            checked={filter?.allSaleFilter?.isActiveUsers === "inactive"}
                ...
                <div>
                    <Flatpickr
                        ...
                        name="date_range"
                        value={filter.allSaleFilter.dateRange}
                ...
                <div className='flex gap-4'>
                    {['treemap', 'area', 'column', 'table'].map(chartType => (
                        <button
                            key={chartType}
                            className={`${filter?.allSaleFilter?.chart === chartType ? "bg-[#1abc9c]" : "bg-[red]/50"} btn btn-sm shadow`}
                ...
                <div className='flex justify-around mt-4  py-1'>
                    ...
                    <div className='flex-1 text-center'>
                        <div>
                            ...
                            <br />
                            <small className='font-bold'>Peak Sales Day</small>
                        </div>
                    </div>
                    <div className='flex-1 text-center'>
