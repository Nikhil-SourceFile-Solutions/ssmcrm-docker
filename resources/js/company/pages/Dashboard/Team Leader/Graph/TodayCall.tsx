...
export default function TodayCall({
    todayCallData, todayCallFilter, setTodayCallFilter, graphLoad
}) {
    ...
    const uniqueVisitorSeries: any = {
        series: [
            {
                name: 'Calls',
                data: todayCallData?.map((l: any) => l.calls),
                type: 'column',
            },
        ...
        options: {
            ...
            xaxis: {
                categories: todayCallData?.map((l: any) => l.name),
                axisBorder: {
                    show: true,
    ...
    return (
        <div className="panel h-full p-0 lg:col-span-2">
            <div className="flex items-start justify-between dark:text-white-light p-5 border-b  border-white-light dark:border-[#1b2e4b]">
                ...
                <div className='flex gap-5'>
                    <label className="inline-flex">
                        <input type="radio" name="default_radio2" value={'active'} className="form-radio" defaultChecked={todayCallFilter == "active" ? true : false}
                            onClick={() => setTodayCallFilter('active')}
                        />
                        <span>Active Users</span>
                    </label>
                    <label className="inline-flex">
                        <input type="radio" name="default_radio2" value={'inactive'} className="form-radio" defaultChecked={todayCallFilter == "inactive" ? true : false}
                            onClick={() => setTodayCallFilter('inactive')}
                        />
            ...
            {graphLoad ? <PageLoader /> : todayCallData?.length ? (
