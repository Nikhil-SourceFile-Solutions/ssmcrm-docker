...
export default function TodaySale({
    ...
    const { todaySalesFilter, graphReload } = filter;


    const filterUsers = todaySaleData.filterUsers;

    const salesData = todaySaleData?.sales || [];
            ...
            <div className="flex items-center justify-between dark:text-white-light p-3 border-b border-white-light dark:border-[#1b2e4b]">
                ...
                <div>
                    <div className='flex gap-4'>
                        <select
                            ...
                            value={todaySalesFilter?.manager}
                        ...
                        {todaySalesFilter?.manager != "0" && (
                            <select
                                className='form-select form-select-sm w-[150px]'
                                value={todaySalesFilter?.leader}
                                onChange={(e) => handleFilter({ action: 'leader', value: e.target.value })}
                            >
                                <option value="0">All </option>
                                {filterUsers?.filter(user => user.user_type === "Team Leader" && user.manager_id == todaySalesFilter?.manager).map(manager => (
                                    <option key={manager.id} value={manager.id}>{manager.first_name} {manager.last_name}</option>
                                ))}
                            </select>
                        )}
                ...
                <div className='flex gap-5'>
                    <label className="inline-flex">
                        <input
                            ...
                            checked={todaySalesFilter?.isActiveUsers === "active"}
                    ...
                    <label className="inline-flex">
                        <input
                            ...
                            value={'inactive'}
                            className="form-radio"
                            checked={todaySalesFilter?.isActiveUsers === "inactive"}
                ...
                <div className='flex gap-4'>
                    {['treemap', 'area', 'column'].map(chartType => (
                        <button
                            key={chartType}
                            className={`${todaySalesFilter?.chart === chartType ? "bg-[#1abc9c]" : "bg-[red]/50"} btn btn-sm shadow`}
                    ...
                    <button
                        ...
                        onClick={() => handleFilter({ action: "reload", value: !todaySalesFilter?.reload })}
                    >
