...
export default function MonthSale({ monthSaleData, filter, setFilter }) {
    const isDark = useSelector((state: IRootState) => state.themeConfig.theme === 'dark' || state.themeConfig.isDarkMode);
    const isRtl = useSelector((state: IRootState) => state.themeConfig.rtlClass) === 'rtl';


    const { monthSalesFilter, graphReload } = filter;

    const filterUsers = monthSaleData.filterUsers;

    const salesData = monthSaleData?.sales || [];
    const salesValues = salesData.map((l: any) => l.sales);
    const salesNames = salesData.map((l: any) => l.name);

    const columnChart = useMemo(() => ({
        series: [{ name: 'Sales', data: salesValues, type: 'column' }],
        options: {
    ...
    const formattedData = useMemo(() => salesData.map(l => ({ x: l.name, y: l.sales })), [salesData]);
    ...
    return (
        <div className="panel h-full p-0 lg:col-span-2">
            <div className="flex items-center justify-between dark:text-white-light p-3 border-b border-white-light dark:border-[#1b2e4b]">
                <h5 className="font-semibold text-lg">Month Sales</h5>
                <div>
                    <div className='flex gap-4'>
                        <select
                            className='form-select form-select-sm w-[150px]'
                            value={monthSalesFilter?.manager}
                        ...
                        {monthSalesFilter?.manager != "0" && (
                            <select
                                className='form-select form-select-sm w-[150px]'
                                value={monthSalesFilter?.leader}
                                onChange={(e) => handleFilter({ action: 'leader', value: e.target.value })}
                            >
                                <option value="0">All</option>
                                {filterUsers?.filter(user => user.user_type === "Team Leader" && user.manager_id == monthSalesFilter?.manager).map(manager => (
                                    <option key={manager.id} value={manager.id}>{manager.first_name} {manager.last_name}</option>
                                ))}
                            </select>
                        )}
                ...
                <div className='flex gap-5'>
                    <label className="inline-flex">
                        <input
                            ...
                            checked={monthSalesFilter?.isActiveUsers === "active"}
                    ...
                    <label className="inline-flex">
                        <input
                            ...
                            value={'inactive'}
                            className="form-radio"
                            checked={monthSalesFilter?.isActiveUsers === "inactive"}
                ...
                <div className='flex gap-4'>
                    {['treemap', 'area', 'column'].map(chartType => (
                        <button
                            key={chartType}
                            className={`${monthSalesFilter?.chart === chartType ? "bg-[#1abc9c]" : "bg-[red]/50"} btn btn-sm shadow`}
                    ...
                    <button
                        ...
                        onClick={() => handleFilter({ action: "reload", value: !monthSalesFilter?.reload })}
                    >
