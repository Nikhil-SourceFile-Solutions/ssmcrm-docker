...
export default function MonthCall({
    monthCallData, filter, setFilter
}) {
    const isDark = useSelector((state: IRootState) => state.themeConfig.theme === 'dark' || state.themeConfig.isDarkMode);
    const isRtl = useSelector((state: IRootState) => state.themeConfig.rtlClass) === 'rtl';

    const { monthCallFilter } = filter;
    const filterUsers = monthCallData.filterUsers;

    const salesData = monthCallData?.calls || [];
    ...
    return (
        <div className="panel h-full p-0 lg:col-span-2">
            <div className="flex items-center justify-between dark:text-white-light p-3 border-b border-white-light dark:border-[#1b2e4b]">
                <h5 className="font-semibold text-lg">Month Calls</h5>
                <div>
                    <div className='flex gap-4'>
                        <select
                            className='form-select form-select-sm w-[150px]'
                            value={monthCallFilter?.manager}
                        ...
                        {monthCallFilter?.manager != "0" && (
                            <select
                                className='form-select form-select-sm w-[150px]'
                                value={monthCallFilter?.leader}
                                onChange={(e) => handleFilter({ action: 'leader', value: e.target.value })}
                            >
                                <option value="0">All</option>
                                {filterUsers?.filter(user => user.user_type === "Team Leader" && user.manager_id == monthCallFilter?.manager).map(manager => (
                                    <option key={manager.id} value={manager.id}>{manager.first_name} {manager.last_name}</option>
                                ))}
                            </select>
                        )}
                ...
                <div className='flex gap-5'>
                    <label className="inline-flex">
                        <input
                            ...
                            checked={monthCallFilter?.isActiveUsers === "active"}
                    ...
                    <label className="inline-flex">
                        <input
                            ...
                            value={'inactive'}
                            className="form-radio"
                            checked={monthCallFilter?.isActiveUsers === "inactive"}
                ...
                <div className='flex gap-4'>
                    {['treemap', 'area', 'column'].map(chartType => (
                        <button
                            key={chartType}
                            className={`${monthCallFilter?.chart === chartType ? "bg-[#1abc9c]" : "bg-[red]/50"} btn btn-sm shadow`}
                    ...
                    <button
                        ...
                        onClick={() => handleFilter({ action: "reload", value: !monthCallFilter?.reload })}
                    >
