...
export default function Whatsapp({ drawer, setDrawer, selectedRecords }: any) {
    ...
    const fetchWhatsappTemplate = async () => {
        ...
        try {
            ...
            console.log('ddfkdfjkfd', response.data.templates);
    ...
    const AddWhatsappCampaign = async (data: any) => {
        try {
            ...
            const response = await axios({
                method: 'post',
                url: apiUrl + "/api/campaigns/whatsapp",
                data,
                headers: {
            ...
            {isLoading ? <PageLoader /> : (
                ...
                <div>
                    {clients ? (<div className='flex gap-2 items-center bg-white-light/40 p-3 rounded mb-3'>
                        <div><RiWhatsappFill size={24} /></div>
                        <div className='text-[16px] font-bold'>
                            <span className='text-black/70'>Whatsapp Message Sending to <span className='text-black'>{clients}</span> Clients</span>
                        </div>
                    </div>) : null}
                    ...
                    <div className='flex'>
                        ...
                        <form autoComplete="off" action="" method="post" className='flex-1 bg-[#e3e7fc] p-4 rounded h-fit'>
...

                            {
                                template && <div className='mb-4  '>
                                    <div className='bg-[#f1faff] p-4 rounded shadow'>
                                        <p style={{ whiteSpace: 'pre-line' }}>{params.final_template}</p>
                                    </div>
                            ...
                            <div className='flex justify-end gap-5 py-2'>
                                <button type="button" className='btn btn-danger shadow' onClick={() => setDrawer(false)}>Cancel</button>
