...
const location = useLocation();
  const page = location.state?.page || 0;
    ...
    const storedSalesTable = localStorage.getItem('saleTable');
    ...
    const [filter, setFilter] = useState({
        ...
        filterState: parsedSalesTable?.filterState ?? 0,
    });
...
const [salesStates, setSalesStates] = useState([]);
    useEffect(() => {
        fetchSales()
        localStorage.setItem("saleTable", JSON.stringify(filter));
    }, [filter])
        ...
        try {
            ...
            if (response.data.status == "success") {
                ...
                if (response?.data?.saleStatuses) {
                    ...
                    setSalesStates(response.data.saleStates)
                }
                            ...
                            <div className='flex justify-between gap-4  p-4'>
                                <div className='flex gap-3  w-full'>
                                    ...
                                    <div className="flex min-w-[275px]">
                                        <input type="text" defaultValue={filter.search} placeholder="Search Sale By Phone or Name"
                                            className={`form-input ${filter.search ? 'border-2 border-[#31ab89]' : ''}  ltr:rounded-r-none rtl:rounded-l-none`} onChange={(e) => setSearch(e.target.value)} />
                                    ...
                                    <div>
                                        ...
                                        {authUser?.user_type == "Admin" || authUser.user_type == "Accounts" || authUser.user_type == "HR" || authUser.user_type == "Complaince" ? (
                                            <Select
                                                ...
                                                className={`w-[230px] z-10 font-bold ${filter.filterOwner ? 'border-2 border-[#31ab89]' : ''}  rounded-md`}
                                                isLoading={isLoading}
                                                isDisabled={isLoading}
                                                value={allSaleOwners?.find((s: any) => s.value == filter.filterOwner)}
                                                options={allSaleOwners}
                                            />
                                        ) : null}
                                    ...
                                    <div>
                                        <Select placeholder="Select Sale Status" onChange={(e: any) => {
                                            ...
                                            isDisabled={isLoading} className={`w-[230px] z-10 font-bold ${filter.filterStatus ? 'border-2 border-[#31ab89]' : ''}  rounded-md`}
                                            value={salesStatuses?.find((s: any) => s.value == filter.filterStatus)} options={salesStatuses}
                                        />
                                    </div>
                                     <div>
                                        <Select placeholder="Select Sale state" onChange={(e: any) => {
                                            setFilter({ ...filter, filterState: e.value, page: 1 })
                                            localStorage.setItem("saleFilterState", e.value);
                                        }} className='w-[200px] z-10' value={salesStates?.find((s: any) => s.value == filter.filterState)} options={salesStates}
                                        />
                                    </div>
                                </div>
