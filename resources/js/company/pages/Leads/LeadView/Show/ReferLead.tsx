...
export default function ReferLead({ referModal, setReferModal, lead_id }: any) {
    ...
    const validate = () => {
        ...
        let errors = {};
        if (!params.to_id) {
            errors = { ...errors, to_id: "employess is required" };
        }
        setErros(errors);
    ...
    return (
        <div>
            <Transition appear show={referModal} as={Fragment}>
                <Dialog
                    as="div"
                    open={referModal}
                    onClose={() => {
                            ...
                            >
                                <Dialog.Panel className="panel border-0 py-1 px-4 rounded-lg overflow-hidden w-full max-w-sm my-8 text-black dark:text-white-dark">
                                    <div className="flex items-center justify-between p-5 font-semibold text-lg dark:text-white">
                                        <h5>Refer Client</h5>
                                        <button type="button" onClick={() => setReferModal(false)} className="text-white-dark hover:text-dark">
