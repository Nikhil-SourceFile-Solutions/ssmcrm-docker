...
export default function WhatsApp({ lead_id, templates, payments, setBankModel }) {
    ...
    useEffect(() => {
        if (params.upi) {
            const upi1: any = payments?.find((t: any) => t.id == params.upi);
            // console.log(upi1)
            const template = templates?.find((temp) => temp.template_type == "Upi Details");
            const { name, upi, image } = upi1;
            const fields = [name, upi, image];
        ...
        <div>
            <section className="flex gap-2 overflow-y-auto overflow-x-hidden perfect-scrollbar mt-5">
                ...
                <div className='flex gap-4 justify-end'>
                    ...
                    <h1 className=' mb-3 font-bold text-lg' >Bank Details</h1>
                ...
                <BankDetailsPreview
                 one={params.bank_final_template}
                 two={params.upi_final_template}
                //  tab={tab}
